...
static struct option long_options[] = { { "size", required_argument, 0, 's' },
  { "times", required_argument, 0, 't' }, { "help", required_argument, 0, 'h' }, { 0, 0, 0, 0 } };
...
void usage()
{
  ...
  printf("Options:\n");
  printf(
      "  -s, --size=SIZE           The size of buffer sent or received at a "
      "time\n");
  ...
  while (1) {
    ...
    switch (option) {
        ...
        while (count < times) {
          ...
          while (num == -1 || num == 0) {  // Wait to receive data
            ...
            if (num > 0) {
              if (strncmp(rx_buffer, &tx_buffer[sum], num)) {
                fprintf(stderr,
                    "Failed! The sent and received buffers are different, "
                    "count:%d\n",
                    count);
                break;
              }
      ...
      case '5':
        printf("\nstart speed test\n");
        printf(
            "Calculate the speed of writing %zu bytes and reading back %zu "
            "bytes\n",
            length, length);
